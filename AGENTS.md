...
- Required environment variables for local app startup are: `DJANGO_SECRET_KEY`, `SOCIAL_AUTH_TWITTER_KEY`, `SOCIAL_AUTH_TWITTER_SECRET`, `TWITTER_ACCESS_TOKEN`, and `TWITTER_ACCESS_TOKEN_SECRET`; `DJANGO_DEBUG` and `DJANGO_ALLOWED_HOSTS` are optional controls.
- Django 1.6.11 has 18 known vulnerabilities in the direct dependency audit. Treat this repository as archival and unsuitable for live deployment.
